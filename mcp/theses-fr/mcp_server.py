...
Exposes theses.fr — the French national register of doctoral theses, run by
ABES — to agent clients: defended theses, theses in preparation, the people
attached to them, and the facet values a query accepts. Public and anonymous:
no key, no credential, nothing to configure but the transport.

Three API facts shape every tool here, all verified against the live service:
  - the documented `filtres` parameter is inert, so all filtering goes through
    the Lucene `q`;
  - search hits carry no résumé, which lives on the record endpoint only;
  - `codeEtab`, not `nnt:*CODE*`, is the establishment filter — a thesis in
    preparation has no NNT, so the wildcard drops all of them.
...
what a load-balanced or multi-worker deployment needs. Incompatible with sse.
  ...
  uv run mcp_server.py --host 0.0.0.0 --port 8017 --transport http
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="theses.fr MCP server",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--host", default=os.environ.get("MCP_HOST", "0.0.0.0"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("MCP_PORT", "8017")))
    parser.add_argument(
        "--transport",
