"""OpenAlex MCP server — Modal deployment.
...
Tools served: `search_works`, `search_semantic`, `lookup_by_doi`,
`get_citing_works`, `classify_text`, `resolve_entity`, `browse_topics`,
`group_by`, `translate_query`.

  # Ephemeral deployment that reloads on save
  uvx modal serve mcp/openalex/modal/mcp_server_stateless.py

  # List the deployed tools (and optionally call one)
  uvx modal run mcp/openalex/modal/mcp_server_stateless.py::test_tool

  # Persistent deployment
  uvx modal deploy mcp/openalex/modal/mcp_server_stateless.py

The MCP endpoint is the printed URL with `/mcp/` appended:

    https://<workspace>--smartbiblia-mcp-openalex-web.modal.run/mcp/
...
Environment: OPENALEX_API_KEY — optional. Since February 2026 OpenAlex meters
usage as a daily budget and ignores `mailto`: anonymous access gets $0.10/day, a
free key $1.00/day, and this deployment shares one budget across all its
visitors. OPENALEX_API_URL — optional, for a mirror or a proxy.
"""

import modal

APP_NAME = "smartbiblia-mcp-openalex"
...
#     modal secret create smartbiblia-openalex OPENALEX_API_KEY=...
# Left out, the server runs on its defaults — which is a working deployment,
# on the $0.10/day anonymous budget.
SECRETS: list[modal.Secret] = []
# SECRETS = [modal.Secret.from_name("smartbiblia-openalex")]
