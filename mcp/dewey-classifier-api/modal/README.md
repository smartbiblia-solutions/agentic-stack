# Dewey classifier MCP server on Modal
...
Open the [MCP inspector](https://github.com/modelcontextprotocol/inspector) and
enter the URL of the MCP server that was printed by the `modal deploy` command,
suffixed with `/mcp/`:
...
uvx modal run mcp/dewey-classifier-api/modal/mcp_server_stateless.py::test_tool \
    ...
    --arguments '{"texts": ["Histoire politique de Buenos Aires au XIXe siècle"], "top_k": 3}'
```
...
```json
{
  "mcpServers": {
    "dewey-classifier-api": {
      "url": "https://<workspace>--smartbiblia-mcp-dewey-classifier-api-web.modal.run/mcp/"
    }
  }
}
```
...
|---|---|---|
| `DEWEY_API_URL` | yes | Base URL of the classifier. The public SmartBibl.IA deployment is a valid value; point it at your own to keep texts in house. |
| `DEWEY_API_KEY` | no | Sent as `X-API-Key`. Never a tool argument, never returned. |
...
`GET` SSE stream, so no server-initiated messages and no resumability. Both tools
here are request/response calls, so nothing is lost.
...
tools, and every call spends your Modal credits and the classifier's capacity.
...
This deployment is only a proxy: **the classification still happens in the Dewey
API**, and the first call against a cold classifier takes tens of seconds while it
downloads its embedding model and builds the taxonomy index. The client allows a
120 s upstream timeout, so the Modal function is given a 600 s limit rather than
the 300 s default.
