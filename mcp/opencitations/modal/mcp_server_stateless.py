"""OpenCitations MCP server — Modal deployment.
...
Tools served: `get_citation_counts`, `get_citations`, `get_references`, `lookup_metadata`, `list_works_by_person`.

  # Ephemeral deployment that reloads on save
  uvx modal serve mcp/opencitations/modal/mcp_server_stateless.py

  # List the deployed tools (and optionally call one)
  uvx modal run mcp/opencitations/modal/mcp_server_stateless.py::test_tool

  # Persistent deployment
  uvx modal deploy mcp/opencitations/modal/mcp_server_stateless.py

The MCP endpoint is the printed URL with `/mcp/` appended:

    https://<workspace>--smartbiblia-mcp-opencitations-web.modal.run/mcp/
...
Environment: OPENCITATIONS_API_KEY — optional, only raises the 180 req/min
anonymous rate limit. Without it the server runs anonymously.
"""

import modal

APP_NAME = "smartbiblia-mcp-opencitations"
...
# SECRETS = [modal.Secret.from_name("smartbiblia-opencitations")]
