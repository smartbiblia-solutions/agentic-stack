...
Exposes HAL (Hyper Articles en Ligne), the French national open repository, over
the MCP protocol. Every call goes to the public CCSD API, no key, no account:

  - documents  https://api.archives-ouvertes.fr/search/
  - references https://api.archives-ouvertes.fr/ref/       (AureHAL)

The search API is a Solr index: the same query language, the same `fq` / `fl` /
`facet.*` / `group.*` parameters, and a field naming scheme where the **suffix
declares what a field can do** (see the `hal_field_reference` prompt-free table
in `search_hal`'s docstring). This server is a thin transport adapter over it —
it builds the URL, retries transient failures and normalizes the answer; it adds
no ranking, no scoring and no domain logic of its own.

Four ways to launch it:

  # 1. Zero-install — run straight from GitHub (uv fetches everything)
  uv run https://raw.githubusercontent.com/smartbiblia-solutions/agentic-stack/main/mcp/hal/mcp_server.py \
      --transport stdio

  # 2. Local stdio — the client owns the process (desktop/IDE)
  uv run /path/to/mcp/hal/mcp_server.py --transport stdio

  # 3. Local HTTP — one server, several clients connected by URL
  uv run /path/to/mcp/hal/mcp_server.py --host 0.0.0.0 --port 8016 --transport http

  # 4. Stateless HTTP — no session affinity, for a replicated deployment
  uv run /path/to/mcp/hal/mcp_server.py --transport http --stateless

Options:
    --host          TEXT    Bind host                    [default: 0.0.0.0]
    --port          INT     Bind port                    [default: 8016]
    --transport     TEXT    stdio | http | sse           [default: http]
                            ("streamable-http" is an accepted alias of "http")
    --stateless             Stateless HTTP: a new transport per request, so no
                            session is pinned to a replica. Rejected with sse.
    --http-timeout  FLOAT   Per-request timeout (s)      [default: 20.0]
    --max-retries   INT     Attempts per request         [default: 3]
    --backoff-base  FLOAT   Exponential backoff base     [default: 1.0]
    --backoff-factor FLOAT  Backoff multiplier           [default: 2.0]
    --jitter-max    FLOAT   Max jitter per retry (s)     [default: 0.25]
    --trace                 Include the HTTP log in every response

Companion skill: `skills/search-records-hal` — same source, same normalization,
command line instead of protocol.
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="HAL MCP server",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--host",           default=os.environ.get("MCP_HOST", "0.0.0.0"))
    p.add_argument("--port",           type=int, default=int(os.environ.get("MCP_PORT", "8016")))
    ...
    p.add_argument("--stateless",      action="store_true",
                   ...
                   help="Stateless HTTP: one transport per request, no session affinity")
    ...
    ns = p.parse_args()
    # FastMCP raises on this combination; fail here instead, with the usage text.
...
if __name__ == "__main__":
    if args.transport == "stdio":
        # Launched by a desktop/IDE client over stdin/stdout.
        # host/port have no effect in this mode.
        mcp.run(transport="stdio")
    else:
        # stateless_http=True rebuilds a transport per request: no session is
        # pinned to a replica, which is what a deployment behind a load balancer
        # or with several uvicorn workers requires. Off by default — a single
        # process is cheaper with state, and stdio clients never reach here.
        mcp.run(
