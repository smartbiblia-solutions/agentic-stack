...
repository operated by the CCSD, or list its portals.

The **canonical MCP endpoint is `mcp_server.py`** one folder up, with the full
tool set (`search_hal`, `list_portals`, `lookup_reference`), the complete field
reference, pivot facets, grouping, paging and no tightened result limit. This
app exposes two of them at `/gradio_api/mcp/sse`; that endpoint is demo-grade
and secondary. Set `GRADIO_MCP_SERVER=false` wherever the real server is
...
`facet_limit` at 200 instead of 500.
...
- **The field suffix decides what a field can do.** `_t` is searchable but not
  ...
  a field's capability returns zero results **with no error** — the third
  example in the search tab demonstrates exactly that.

## Debugging a failing call

Both tabs carry a **🔍 Debug** accordion under the results. It shows the exact
